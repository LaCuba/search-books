...
const Modal = () => {
  ...
  const book =
    bookKey && snippets && snippets.filter((snippet) => snippet.key === bookKey)

  return (
    <>
      {book && (
        <div
          className={book ? styles.modalActive : styles.modal}
          onClick={() => dispatch(setBookKey(null))}
        >
            ...
            <div className={styles.info}>
              ...
              {book[0].publish_date && (
                <div className={styles.datePublish}>
                  <b>Publish date: </b>
                  <p>{book[0].publish_date[0]}</p>
                </div>
              )}
              {book[0].publisher && (
                <div className={styles.publisher}>
                  <b>Publisher: </b>
                  <p>{book[0].publisher[0]}</p>
                </div>
              )}
              {book[0].isbn && (
